...
export function FhirClientProvider(props){
  ...
  useEffect(function(){
    async function initializeClient(){
      console.info("SMART.ready()", await SMART.ready());

      await SMART.ready()
        .then(smartClient => {
          console.log("===========================================================================")
          console.log("SMART ON FHIR - FhirClientProvider")
          console.log("SMART.ready() - smartClient", smartClient)
          ...
          if(state){
            ...
            //   httpOptions.headers["Authorization"] = "Basic " + atob(get(smartOnFhirConfig, 'client_id') + ":" + get(smartOnFhirConfig, 'client_secret'));
            // } 
            ...
            console.log('FhirClientProvider.patientUrl.httpOptions', httpOptions);
            ...
            if(accessToken){
              if(metadataUrl){  
                ...
                HTTP.get(metadataUrl, httpOptions, function(error, conformanceStatement){
                  if(error){
                    console.error('FhirClientProvider.metadataUrl.get().error', error)
                  }
                  if(conformanceStatement){
                    ...
                    // TODO:  fetch well known smart config
                    console.log('-------------------------------------------------------')
  ...
  function fetchWellKnownSmartConfig(){
    console.log('FETCHING WELL-KNOWN SMART CONFIG')          
  ...
  function exchangeCodeForAccessToken(wellKnownSmartConfig){
    ...
    console.log('exchangeCodeForAccessToken.searchParams', searchParams)

    if(searchParams.size === 0){
      searchParams = new URLSearchParams(Session.get('redirectLandingUrl'));
    }

    let decoded = jwt.decode(searchParams.get('code'));
    console.log('exchangeCodeForAccessToken.decoded', decoded)

    let clientSecret = get(Meteor, 'settings.public.smartOnFhir[0].client_secret', '');
    ...
    if(clientSecret){
      messageHeaders["Authorization"] = 'Basic ' + btoa(get(Meteor, 'settings.public.smartOnFhir[0].client_id', '') + ':' + encodeURIComponent(clientSecret)); 
    } else {
      payload.client_id = get(Meteor, 'settings.public.smartOnFhir[0].client_id', '');
    }
    ...
    if(searchParams.get('code')){
      ...
      console.warn('No code found in searchParams.  Cannot exchange for access token.')
    }
  }
