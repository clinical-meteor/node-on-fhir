...
// import AnswerQuestionsWorker from 'worker-loader!./answerQuestions.worker';
// import EchoQuestionsWorker from 'worker-loader!./echoQuestions.worker';



export function FillbotPage(props){
  ...
  useTracker(function(){
    setQuestionnaireResponse(QuestionnaireResponses.findOne({id: get(Meteor, 'settings.public.modules.fillbot.defaultQuestionnaireResponseId', "")}))
  }, [])
  ...
  useEffect(function(){
    ...
    if(defaultQuestionnaireResponseId){
      let defaultQuestionnaireResponse = QuestionnaireResponses.findOne({id: defaultQuestionnaireResponseId});  
      if(defaultQuestionnaireResponse){
        setQuestionnaireResponse(JSON.stringify(defaultQuestionnaireResponse, null, 2));
      } else {
        console.log('No defaultQuestionnaireResponse found for ' + defaultQuestionnaireResponseId)
      }
    }
  ...
  function echoQuestions(){
    console.log("=========================================================================================")
    console.log("ECHOING QUESTIONS")

    // let worker = new EchoQuestionsWorker();

    // worker.postMessage({ questions: serializedResponse, medicalHistory: textNormalForm });

    // worker.onmessage = (event) => {
    //   console.log(event.data); // 'Done processing questions'
    // };
  ...
  function answerQuestions(){
    ...
    console.log('typeof serializedResponse', typeof serializedResponse)
    console.debug('serializedResponse', serializedResponse)
    // worker.postMessage({ questions: serializedResponse, medicalHistory: textNormalForm });
    ...
    let parsedQuestionnaireResponse;
    if(typeof serializedResponse === "string"){
      parsedQuestionnaireResponse = JSON.parse(serializedResponse);
      // if(serializedResponse.includes('\n')){
      //   parsedQuestionnaireResponse = serializedResponse.split('\n');
      // } else {
      //   parsedQuestionnaireResponse = JSON.parse(serializedResponse);
      // }
    } else if(typeof serializedResponse === "object"){
      parsedQuestionnaireResponse = serializedResponse;
    }
      ...
      parsedQuestionnaireResponse.forEach(function(item, index){
        if(typeof item !== "undefined"){
          ...
          if(nextQuestion){
            ...
            if(get(nextQuestion, 'type') === "choice"){
              ...
              setTimeout(async function(){
                ...
                if(nextQuestion){      
                  ...
                  let nextQuestionAnswerOptions = get(nextQuestion, 'answerOption', []);

                  console.log('nextQuestionAnswerOptions', nextQuestionAnswerOptions);
                  console.log("")

                  let answerOptionsText = "";
                  let llmTextResponse = "";
                  let llmMultipleChoiceResponse = "";

                  answerOptionsText = JSON.stringify(nextQuestionAnswerOptions);
                  ...
                  let llmObjectResponse = await askMultipleChoiceQuestionChain.invoke({
                    jsonInput: questionToAskTheOracle
                  })

                  let parsedAnswerResponse;
                  if(typeof llmObjectResponse === "string"){
                    parsedAnswerResponse = JSON.parse(llmObjectResponse);
                  } else if(typeof llmObjectResponse === "object"){ 
                    parsedAnswerResponse = llmObjectResponse;
                  }
                  
                  llmTextResponse = get(parsedAnswerResponse, 'explanation', '');
                  llmMultipleChoiceResponse = get(parsedAnswerResponse, 'answerChoice', '');
                  ...
                  console.log(llmMultipleChoiceResponse);


                  console.log("")
                  console.log('ANSWER');
                  console.log(llmTextResponse);
                  console.log("------------------------------------------------------------------")
                  ...
                  if(Array.isArray(llmMultipleChoiceResponse)){
                    newItem.answer = llmMultipleChoiceResponse;
                  }

                  newItem.answer.push({
                    valueString: llmTextResponse
                  })
            ...
            } else if(["question", "text", "date", "dateTime", "time", "string", "url", "reference"].includes(get(nextQuestion, 'type'))){
              ...
              setTimeout(async function(){
                ...
                if(nextQuestion){      
                  ...
                  llmTextResponse = await askQuestionChain.invoke({
                    jsonInput: questionToAskTheOracle,
                  })
  ...
  function prepQuestions(){
    console.log('Preparing questions...', questionnaireResponse)

    let completedQuestionArray = [];


    // is this ndjson?
    let questionItemsArray = [];
    if(typeof questionnaireResponse === "string"){
      questionItemsArray = get(JSON.parse(questionnaireResponse), 'item', [])
    } else if(typeof questionnaireResponse === "object"){
      questionItemsArray = get(questionnaireResponse, 'item', [])
    }
    console.log('questionItemsArray', questionItemsArray)
    setSerializedResponse(JSON.stringify(questionItemsArray, null, 2));

    console.log("------------------------------------------------------------------")

    if(Array.isArray(questionItemsArray)){
      questionItemsArray.forEach(function(item){
        console.log('item', item)
        if(item){
          if(get(item, 'type') !== "display"){
            completedQuestionArray.push(get(item, 'text'));
          } 
        }      
      });
    }
  ...
  function createQuestionnaireResponse(){
    ...
    if(questionnaireResponse && typeof questionnaireResponse === "string"){
      newQuestionnaireResponse = JSON.parse(questionnaireResponse);
    } else {

      if(typeof questionnaire === "string"){
        parsedQuestionnaire = JSON.parse(questionnaire);
      } else if(typeof questionnaire === "object"){
        parsedQuestionnaire = questionnaire;
      }
  
      console.log('Questionnaire', parsedQuestionnaire)
  
      newQuestionnaireResponse = {
        "_id": get(parsedResponse, '_id', Random.id()),
        "id": get(parsedResponse, 'id', Random.id()),
        "responseType": "QuestionnaireResponse",
        "questionnaire": get(parsedQuestionnaire, "id"),
        "title": get(parsedQuestionnaire, "title"),
        "status": "completed",
  ...
  function saveQuestionnaireResponse(){
    ...
    let parsedResponse;
    if(typeof questionnaireResponse === "string"){
      parsedResponse = JSON.parse(questionnaireResponse);
    } else if(typeof questionnaireResponse === "object"){
      parsedResponse = questionnaireResponse;
    }
    console.log('parsedResponse', parsedResponse);


    if(QuestionnaireResponses.findOne(get(parsedResponse, '_id'))){
      QuestionnaireResponses._collection.update(get(parsedResponse, '_id'), {$set: parsedResponse}, {filter: false, validate: false});
    } else {
      QuestionnaireResponses._collection.insert(parsedResponse, {filter: false, validate: false});
    }

  }

  function backToHomepage(){
    props.history.replace(get(Meteor, 'settings.public.defaults.dataExporterNextPageUrl', '/'));
  }
  ...
  if(ndjsonString){
    patientMedicalHistory = <CardContent>
      <AceEditor
        ...
        name="synthesisEditor"
        editorProps={{ $blockScrolling: true }}
        value={typeof ndjsonString === "string" ? ndjsonString : JSON.stringify(ndjsonString, null, 2)}
  ...
  let returnContents = <Container maxWidth="xxl" style={{paddingTop: '80px', paddingBottom: '80px'}}>
    <Box sx={{ width: '100%', borderBottom: 1, borderColor: 'divider', marginTop: '80px' }}>
      <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">        
        <Tab label="Answer Questionnaire" {...a11yProps(0)} />
        <Tab label="Health Record" {...a11yProps(1)} />
        <Tab label="Ask a Question" {...a11yProps(2)} />
        <Tab label="Questionaire Prep" {...a11yProps(3)} />
      </Tabs>
    </Box>
    ...
    <CustomTabPanel value={value} index={0}>
      <Grid container spacing={2} style={{paddingTop: '40px'}}>        
        <Grid item md={2}>
          <Card>
            <CardHeader title="Clinical Text Normal Form"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
        ...
        <Grid item md={2}>
          <Card>
            <CardHeader title="New Questionnaire Response"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof questionnaireResponse === "string" ? questionnaireResponse : JSON.stringify(questionnaireResponse, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />                            
              
              <DynamicSpacer />              
              <Button fullWidth variant="contained" color="primary" onClick={prepQuestions} disabled={openAiApiKey ? false : true}>
                Prep Questions
              </Button>
            </CardContent>
          </Card>      
        ...
        <Grid item md={2}>
          <Card>
            <CardHeader title="Questions to Ask"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof listOfCompletedQuestions === "string" ? listOfCompletedQuestions : JSON.stringify(listOfCompletedQuestions, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />                            
              ...
              >
                ...
                <MenuItem value={10}>ChatGPT</MenuItem>
                <MenuItem disabled value={20}>Claud</MenuItem>              
                <MenuItem disabled value={30}>Mistral</MenuItem>
                <MenuItem disabled value={40}>Llama</MenuItem>
                <MenuItem disabled value={50}>Medtronic</MenuItem>                
                <MenuItem disabled value={60}>Gemini</MenuItem>
              </Select>
              <DynamicSpacer />
              <Button fullWidth variant="contained" color="primary" onClick={answerQuestions} disabled={openAiApiKey ? false : true}>
                Answer Questions
              </Button>
              <DynamicSpacer />
              <Button fullWidth variant="contained" color="primary" onClick={echoQuestions} disabled={openAiApiKey ? false : true}>
                Echo Questions
              </Button>
            </CardContent>
          </Card>      
        ...
        <Grid item md={2}>
          <Card>
            <CardHeader title="Answers"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
        ...
        <Grid item md={2}>
          <Card>
            <CardHeader title="Serialized Answers"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof serializedAnswers === "string" ? serializedAnswers : JSON.stringify(serializedAnswers, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
              <DynamicSpacer />
              <Button fullWidth variant="contained" color="primary" onClick={createQuestionnaireResponse}>
        ...
        <Grid item md={2}>
          <Card>
            ...
            <CardContent>
              ...
              />              
              <DynamicSpacer />
              <Button fullWidth variant="contained" color="primary" onClick={saveQuestionnaireResponse} disabled={openAiApiKey ? false : true}>
                Save Questionnaire Response
              </Button>
              <DynamicSpacer />
              <Button fullWidth variant="contained" color="primary" onClick={backToHomepage} disabled={openAiApiKey ? false : true}>
                Back to Form 8500-8 Homepage
              </Button>
            </CardContent>
          </Card>      
        </Grid>
    ...
    <CustomTabPanel value={value} index={1}>
      <Grid container spacing={2} style={{paddingTop: '40px'}}>
        <Grid item md={cardWidth}>
          <Card>
            <CardHeader title="Workflow" ></CardHeader>
          ...
          <Card>
            ...
            <CardContent>
              <AceEditor
                ...
                editorProps={{ $blockScrolling: true }}
                value={typeof textNormalForm === "string" ? textNormalForm : JSON.stringify(textNormalForm, null, 2)}
                style={{width: '100%', position: 'relative', height: '620px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />            
              <DynamicSpacer />
              <Button disabled={textNormalForm ? true : false} fullWidth variant="contained" color="primary" onClick={handeGenerateNarativeSummary} disabled={(openAiApiKey && selectedPatient) ? false : true}>
        ...
        <Grid item md={cardWidth}>
          <Card>
            <CardHeader title="Narrative Summary"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof patientNarrative === "string" ? patientNarrative : JSON.stringify(patientNarrative, null, 2)}
                style={{width: '100%', position: 'relative', height: '660px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
            </CardContent>
          </Card>      
          <br/>
    ...
    <CustomTabPanel value={value} index={2}>
      <Grid container spacing={2} style={{paddingTop: '40px'}}>        
        ...
        <Grid item md={3}>
          <Card>
            <CardHeader title="Global Prompt"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
                wrapEnabled={true}
                onChange={this.onChangeGlobalPrompt}
                name="globalPrompt"
                editorProps={{ $blockScrolling: true }}
                value={typeof globalPrompt === "string" ? globalPrompt : JSON.stringify(globalPrompt, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
            </CardContent>
          </Card>      
        ...
        <Grid item md={3}>
          <Card>
            <CardHeader title="Ask a Question"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                wrapEnabled={true}
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                defaultValue={typeof questionText === "string" ? questionText : JSON.stringify(questionText, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
              <br />
              <br />
              <Button fullWidth variant="contained" color="primary" onClick={askQuestion} disabled={openAiApiKey ? false : true}>
                Ask Question
              </Button>
            </CardContent>
          </Card>      
        ...
        <Grid item md={3}>
          <Card>
            <CardHeader title="Question Response"></CardHeader>
            <CardContent>
              <AceEditor
                mode="text"
                theme="github"
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof questionResponse === "string" ? questionResponse : JSON.stringify(questionResponse, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
            </CardContent>
          </Card>      
          ...
          <Card>
            ...
            <CardContent>
              ...
              <AceEditor
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof questionnaire === "string" ? questionnaire : JSON.stringify(questionnaire, null, 2)}
          ...
          <Card>
            ...
            <CardContent>
              <AceEditor
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof serializedQuestionnaire === "string" ? serializedQuestionnaire : JSON.stringify(serializedQuestionnaire, null, 2)}
                style={{width: '100%', position: 'relative', height: '400px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
              <DynamicSpacer />
          ...
          <Card>
            ...
            <CardContent>
              <AceEditor
                ...
                editorProps={{ $blockScrolling: true }}
                value={typeof stringifiedQuestionnaire === "string" ? stringifiedQuestionnaire : JSON.stringify(stringifiedQuestionnaire, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
              <DynamicSpacer />
          ...
          <Card>
            ...
            <CardContent>
              <AceEditor
                ...
                name="synthesisEditor"
                editorProps={{ $blockScrolling: true }}
                value={typeof serializedResponse === "string" ? serializedResponse : JSON.stringify(serializedResponse, null, 2)}
                style={{width: '100%', position: 'relative', height: '600px', minHeight: '200px', backgroundColor: '#f5f5f5', borderColor: '#ccc', borderRadius: '4px', lineHeight: '16px', marginTop: '20px'}}        
              />              
              <DynamicSpacer />
